"""Local-only telemetry for agent-lint usage tracking.

Opt-in via AGENT_LINT_TELEMETRY=1 environment variable.
...
_ENV_VAR = "AGENT_LINT_TELEMETRY"
_DEFAULT_DIR = Path.home() / ".agent-lint"
...
def _telemetry_dir() -> Path:
    """Get the telemetry directory (respects AGENT_LINT_DIR env var)."""
    return Path(os.environ.get("AGENT_LINT_DIR", str(_DEFAULT_DIR)))
...
class TelemetryStore:
    ...
    def __init__(self, db_path: Path) -> None:
        ...
        try:
            ...
            raise AgentAuditError(f"Failed to initialize telemetry database: {e}") from e
    ...
    def record(
        ...
        try:
            ...
            raise AgentAuditError(f"Failed to record telemetry event: {e}") from e
